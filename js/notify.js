...
const LABEL = { 0: '現在', 10: '10 分鐘後', 60: '1 小時後', 180: '3 小時後', 1440: '明天', 4320: '3 天後', 10080: '1 週後' };
...
export async function reschedule() {
  ...
  const list = [];
  // 所有行事曆的未完成任務都要提醒（不只目前選的那個）
  for (const t of db.all('tasks', x => x.status !== 'done' && x.date && x.reminders?.length)) {
    const [y, m, d] = t.date.split('-').map(Number);
    const [hh, mm] = (t.start_time || fallback).split(':').map(Number);
    const at = new Date(y, m - 1, d, hh, mm).getTime();
    for (const before of t.reminders) {
      const fire = at - before * 60e3;
      if (fire <= now || fire > until) continue;
      list.push({
        id: intId(t.id + '|' + before),
        title: t.title,
        body: [LABEL[before] ?? `${before} 分鐘後`, fmtWhen(t)].join('・'),
